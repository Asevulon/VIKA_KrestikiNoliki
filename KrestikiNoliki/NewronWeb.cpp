...
void MatrixForm::ORO(OptTurn& ot)
{
	...
	// Входной слой
	Matrix2d input_vector(ot.first); // Форма (9, 1)
	a[0] = input_vector.Transpose(); // (1, 9) для совместимости с матрицами весов
	...
	// Целевой вектор (one-hot encoding правильного хода)
	Matrix2d t(9, 1, 0.0);
	t(ot.second, 0) = 1.0;
	...
	// Градиент для выходного слоя (Softmax + кросс-энтропия)
	delta[last] = y - t;
	...
	// Обновление градиентов для выходного слоя
	dW[last] = delta[last] * a[last].Transpose();
	dFi[last] = delta[last];

	// Распространение градиентов через скрытые слои
	for (int i = last - 1; i >= 0; --i) {
		// Производная ReLU: 1 если z[i] > 0, иначе 0
	...
	// Обновление весов и смещений с учетом скорости обучения (etta)
